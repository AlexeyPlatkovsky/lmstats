...
3. Run `test-change` first. For a significant feature, bug fix, or refactor,
   add a focused test that fails before the production implementation; record
   the reason if a meaningful automated test cannot be written first.
